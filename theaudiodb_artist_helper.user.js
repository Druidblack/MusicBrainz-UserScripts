...
// @version      1.2.1-fanart-extended-fix5-cache
...
(function() {
  ...
  /*** ====== НАСТРОЙКИ ====== ***/
  const LFM_API_KEY    = '7d2b34081acbb4f72c2760c2c8496e29';
  const FANART_API_KEY = '70dad2c260a01c7c7efb06bb68b27553';
  const MAX_ALIASES = 8;
  const MAX_TAGS = 10;
  const WIKI_PREF_KEY = 'adb_wiki_lang'; // 'ru' | 'en'
  const CACHE_TTL_MS = 3600 * 1000; // CACHE: 1 час

  /*** ====== КЭШ (localStorage) ====== ***/
  // CACHE: простые обёртки для JSON/текста на базе localStorage
  ...
  async function cachedGetJSON(url, opts={}) { // opts: { headers?, ttlMs? }
  ...
  async function cachedGetText(url, opts={}) { // opts: { ttlMs? }
    const ttl = typeof opts.ttlMs === 'number' ? opts.ttlMs : CACHE_TTL_MS;
  ...
  async function fetchFanart(mbid) {
    ...
    try { return await cachedGetJSON(`https://webservice.fanart.tv/v3/music/${mbid}?api_key=${FANART_API_KEY}`); } // CACHE
    catch(e){ return null; }
  }
  async function fetchFanartAlbums(mbid) {
    if (!FANART_API_KEY || !mbid) return null;
    try { return await cachedGetJSON(`https://webservice.fanart.tv/v3/music/albums/${mbid}?api_key=${FANART_API_KEY}`); } // CACHE
    catch(e){ return null; }
  }
  ...
  async function fetchLastfm(name) {
    ...
    try { const j = await cachedGetJSON(url); if (j?.artist) return j.artist; } catch(e){ } // CACHE
    return null;
  }
  async function fetchLastfmTopTags(name) {
    ...
    try {
      ...
      const j = await cachedGetJSON(url); // CACHE
  ...
  // ===== Last.fm Biography (/+wiki) =====
  function buildLfmWikiUrl(baseArtistUrl, pageName) {
    if (baseArtistUrl) {
      const trimmed = baseArtistUrl.replace(/\/+$/,'');
      return `${trimmed}/+wiki`;
    }
    // fallback
    return `https://www.last.fm/music/${encodeURIComponent(pageName)}/+wiki`;
  }

  async function fetchLfmBiography(lfmWikiUrl) {
    try {
      const html = await cachedGetText(lfmWikiUrl); // CACHE
      const doc  = new DOMParser().parseFromString(html, 'text/html');
  ...
  function renderPanel({pageName, mbArtist, mbLookup, mbid, links, lfm, lfmTags, lfmMeta, fanart, fanartAlbums}) {
    ...
    el.style.background = '#2b2b2b'; // серый фон (как просили)
    ...
    const tagsLfmHtml = (lfmTags||[]).map(t => `<a href="${esc(t.url)}" target="_blank" rel="noopener">${esc(t.name)}</a>`).join(', ');

    // MB type/gender — только вывод
    const mbTypeRaw   = mbLookup?.type || mbArtist?.type || null;      // e.g. "Person", "Group", ...
    const mbGenderRaw = mbLookup?.gender || mbArtist?.gender || null;  // e.g. "Male", "Female", ...
    ...
    function linkOrDash(label, url) {
      ...
      return `<span>${label}: <a href="${esc(url)}" target="_blank" rel="noopener">${esc(short)}</a></span>`;
    }
    ...
    const artistParts =
      `Background=${cnt.background}, Thumb=${cnt.thumb}, Banner=${cnt.banner}, Logo(HD)=${cnt.logo_hd}, Logo=${cnt.logo_sd}, Music Banner=${cnt.musicbanner}`;
    const albumParts =
      `Album Cover=${ac.albumcover}, CDArt=${ac.cdart}, Album Thumb=${ac.albumthumb}`;

    const fanartInfoHTML = `
      <div style="margin-top:8px;font-size:13px;opacity:.85">
        fanart.tv: ${artistParts} · Albums: ${albumParts}
        ${fanartLink?` · <a target="_blank" rel="noopener" href="${esc(fanartLink)}">artist</a>`:''}
      </div>`;
    ...
    const preferred = (localStorage.getItem(WIKI_PREF_KEY) || 'ru');
      ...
      <div style="display:flex; gap:16px; align-items:flex-start; flex-wrap:wrap">
        <div style="flex:1 1 480px; min-width:320px">
          <div style="font-size:18px; font-weight:700; line-height:1.2">
            ADB Artist Helper — <span style="opacity:.8">для</span> <span style="color:#0b6;">${esc(pageName)}</span>
          </div>
          <div style="margin-top:4px; font-size:13px; opacity:.8">
            Основной источник: <a href="https://musicbrainz.org/" target="_blank" rel="noopener">MusicBrainz</a>
          </div>

          <div style="margin-top:12px; font-size:14px;">
            <div><b>MBID:</b> ${mbid ? `<a href="https://musicbrainz.org/artist/${esc(mbid)}" target="_blank" rel="noopener">${esc(mbid)}</a>` : '—'}
              ${mbid ? `<button id="adb-copy-mbid" style="margin-left:8px; padding:2px 8px; font-size:12px; cursor:pointer;">Copy</button>`:''}
            </div>
            <div style="margin-top:6px"><b>Country:</b> ${esc(mbArtist?.country||mbLookup?.country||'—')}</div>
            ...
            <div style="margin-top:6px"><b>Disambiguation:</b> ${esc(mbArtist?.disambiguation||mbLookup?.disambiguation||'—')}</div>
            <div style="margin-top:6px"><b>Aliases:</b> ${aliasList.length ? esc(aliasList.join(', ')) : '—'}</div>

            ${ hasLfmMetaBlock ? `
              <div style="margin-top:10px; font-size:14px;">
                ${ yearsActive ? `<div><b>Years Active:</b> ${esc(yearsActive)}</div>` : '' }
                ${ foundedIn   ? `<div style="margin-top:6px"><b>Founded In:</b> ${esc(foundedIn)}</div>` : '' }
                ${ born        ? `<div style="margin-top:6px"><b>Born:</b> ${esc(born)}</div>` : '' }
                ${ bornIn      ? `<div style="margin-top:6px"><b>Born In:</b> ${esc(bornIn)}</div>` : '' }
              </div>` : '' }
        ...
        <div style="flex:2 1 520px; min-width:320px">
          ...
          <div style="display:flex; align-items:center; gap:8px; margin-top:2px">
            ...
            <div id="adb-wiki-toggle" style="display:inline-flex; border:1px solid rgba(0,0,0,.15); border-radius:10px; overflow:hidden;">
              <button data-lang="ru" class="adb-wiki-tab" style="padding:4px 10px; font-size:12px; border:none; background:${preferred==='ru'?'#e6f4ea':'transparent'}; cursor:pointer;">RU</button>
              <button data-lang="en" class="adb-wiki-tab" style="padding:4px 10px; font-size:12px; border:none; border-left:1px solid rgba(0,0,0,.15); background:${preferred==='en'?'#e6f4ea':'transparent'}; cursor:pointer;">EN</button>
            </div>
          </div>
          <div id="adb-wiki-extract" style="font-size:13px; white-space:pre-wrap; background:rgba(0,0,0,.03); padding:10px 12px; border-radius:10px; max-height:220px; overflow:auto; border:1px solid rgba(0,0,0,.06); margin-top:8px">
            Загрузка Wikipedia…
          </div>
          ...
          <div id="adb-lfm-bio" style="font-size:13px; white-space:pre-wrap; background:rgba(0,0,0,.03); padding:10px 12px; border-radius:10px; max-height:260px; overflow:auto; border:1px solid rgba(0,0,0,.06); margin-top:8px">
    ...
    // вставить панель сразу после блока с textarea биографии (GB/RU)
...
if (bioRow) {
  ...
  // фоллбек: как раньше — перед футером
  ...
  function setActiveWikiButton(lang) {
    ...
    buttons.forEach(b => b.style.background = (b.dataset.lang === lang ? '#e6f4ea' : 'transparent'));
  }
  async function ensureWikiContent({ name, qid, lang }) {
    ...
    src.innerHTML = `Источник: <a target="_blank" rel="noopener" href="${esc(link)}">${esc(data.title)}</a>`;
  }
  async function ensureLfmBiography({ baseArtistUrl, pageName }) {
    const box = $('#adb-lfm-bio');
    const src = $('#adb-lfm-bio-source');
    if (!box || !src) return;
    const wikiUrl = buildLfmWikiUrl(baseArtistUrl, pageName);
    box.textContent = 'Загрузка биографии с Last.fm…';
    const text = await fetchLfmBiography(wikiUrl);
    box.textContent = text || '—';
    src.innerHTML = `Источник: <a target="_blank" rel="noopener" href="${esc(wikiUrl)}">${esc(wikiUrl.replace(/^https?:\/\//,''))}</a>`;
  }
  ...
  async function main() {
    ...
    // LFM meta (Years Active / Founded In / Born / Born In)
    const lastfmLink = links.lastfm || (lfm?.url ? lfm.url : `https://www.last.fm/music/${encodeURIComponent(pageName)}`);
    ...
    // WIKI: инициализация + кнопки
    const preferred = (localStorage.getItem(WIKI_PREF_KEY) || 'ru');
    const toggle = $('#adb-wiki-toggle');
    ...
    ensureWikiContent({ name: pageName, qid, lang: preferred });

    // LFM Biography
    await ensureLfmBiography({ baseArtistUrl: lastfmLink, pageName });
  }
